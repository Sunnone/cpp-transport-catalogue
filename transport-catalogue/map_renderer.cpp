...
std::vector<svg::Polyline> MapRenderer::RenderRouteLines(const std::map<std::string_view, BusPtr>& buses, const SphereProjector& sp) const { 
    std::vector<svg::Polyline> result; 
    size_t color_num = 0; 
    for (const auto& [bus_number, bus] : buses) { 
        if (bus->stops.empty()) continue; 
        std::vector<StopPtr> route_stops{ bus->stops.begin(), bus->stops.end() }; 
        if (bus->type == RouteType::Straight) route_stops.insert(route_stops.end(), std::next(bus->stops.rbegin()), bus->stops.rend()); 
        svg::Polyline line; 
...
std::vector<svg::Text> MapRenderer::RenderBusLabel(const std::map<std::string_view, BusPtr>& buses, const SphereProjector& sp) const { 
    ...
    for (const auto& [bus_number, bus] : buses) { 
        ...
        if (bus->type == RouteType::Straight && bus->stops[0] != bus->stops[bus->stops.size() - 1]) { 
            svg::Text text2 {text}; 
...
std::vector<svg::Circle> MapRenderer::RenderStopsSymbols(const std::map<std::string_view, StopPtr>& stops, const SphereProjector& sp) const { 
...
std::vector<svg::Text> MapRenderer::RenderStopsLabels(const std::map<std::string_view, StopPtr>& stops, const SphereProjector& sp) const { 
    std::vector<svg::Text> result; 
    svg::Text text; 
    svg::Text underlayer; 
    for (const auto& [stop_name, stop] : stops) { 
...
svg::Document MapRenderer::RenderSVG(const std::map<std::string_view, BusPtr>& buses) const { 
    svg::Document result; 
    std::vector<geo::Coordinates> route_stops_coord; 
    std::map<std::string_view, StopPtr> all_stops; 
    ...
    for (const auto& line : RenderRouteLines(buses, sp)) result.Add(line); 
    for (const auto& text : RenderBusLabel(buses, sp)) result.Add(text); 
    for (const auto& circle : RenderStopsSymbols(all_stops, sp)) result.Add(circle); 
    for (const auto& text : RenderStopsLabels(all_stops, sp)) result.Add(text); 
